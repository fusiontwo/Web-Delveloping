...
public class ArrayEx01 {
	...
	public static void main(String[] args) {
		...
		// 2. 주소 변수 (주소 저장)
		int[] arr = null;  // 주소 변수의 초깃값은 null을 대입한다.
		arr = new int[5];  // 방의 개수
		
		System.out.println(arr);  // 배열의 변수명은 주소(참조, reference)
		System.out.println();
		
		// 값을 넣지 않은 상태에서 초기값은 0이다. (이미 초기화 되어있음.)
...
//		System.out.println(arr[5]);
		...
		for (int i = 0; i < arr.length; i++) {  // for 입력 후, Ctrl + spacebar 자동완성 시 생성
			System.out.println(arr[i]);
		}
		System.out.println();
		...
		// arr에 null을 대입하여 배열 자체를 삭제 가능
		// 연결이 끊어진 arr을 가비지 컬렉터가 보관한다.
		arr = null;
		for (int i = 0; i < arr.length; i++) {
			System.out.println(arr[i]);
		}
